...
**End-to-end MLOps platform for Remaining Useful Life (RUL) classification on turbofan engines**
...
graph TD
    ...
    subgraph Observability["Observability"]
        API -->|structured JSON logs| LOGS[(CloudWatch)]
    end
```

No streaming, no online feature store, no drift-monitoring service: this project used to run all three (a Kafka-fed real-time consumer, a Redis-backed Feast online store materialized every 30 minutes, and an Evidently service watching raw telemetry). All three were removed together — see [Monitoring & Observability](#monitoring--observability) for why, and [Architectural Decisions & Trade-offs](#architectural-decisions--trade-offs) for the infrastructure side of that call.

### Data Flow, in Words

1. **Ingestion.** Historical C-MAPSS batches land in the S3 data lake.
2. **Feature engineering.** Historical data is transformed into fixed-size sliding windows (30 timesteps x 24 features — see the note on window shape below) and written to the **Feast Offline Store** (S3/Parquet), which guarantees point-in-time correctness — no future data ever leaks into a training window. `train_model` runs `feast apply`, which registers the entity/feature-view schema, then pulls features back out through `get_historical_features`. There is no online store: this project does not serve inference from streaming data, so there is nothing to materialize into low-latency storage (see [Monitoring & Observability](#monitoring--observability)).
...
7. **Inference.** The FastAPI service loads the `champion`-aliased model and scaler from MLflow at startup and serves it synchronously over `/predict`, authenticated with an API key (`api/main.py::require_api_key`).
8. **Observability.** Every request is logged as structured JSON, shipped to CloudWatch. There is no drift-monitoring service in this project right now — see [Monitoring & Observability](#monitoring--observability) for why that's a deliberate scoping decision, not an oversight.
...
| **Feature Store** | Feast (offline store only), Apache Parquet | Point-in-time-correct historical training features, eliminating training/serving skew |
...
| **API Serving** | FastAPI, Uvicorn | Synchronous, self-documenting, type-validated, API-key-authenticated inference endpoint |
...
| **Cloud Infrastructure** | AWS VPC, EKS, RDS (PostgreSQL), S3, ECR, Secrets Manager, KMS (customer-managed key), CloudWatch Logs, DynamoDB (Terraform state lock), IAM (OIDC federation, IRSA) | Managed, private-by-default infrastructure with no long-lived static credentials |
| **Local Integration Testing** | Testcontainers, LocalStack | Real, ephemeral Postgres/S3/Secrets Manager containers proving actual integration, at zero cloud cost |
| **Observability** | structlog | JSON-structured logs, directly queryable in CloudWatch Logs Insights |
...
push / merge to main
 ...
 train          →  prepare features + S3 upload + feast apply; train against the full dataset
      ...
      │
      ▼
 build_image    →  build + push images (API, MLflow) to ECR
      │
      ▼
...
**This project does not run a drift-monitoring stack.** That is a deliberate scoping decision, made explicit here rather than left as a silent gap someone has to discover by reading code.

**Why.** Meaningful drift monitoring needs two different things, and conflating them is the single most common way "drift monitoring" ends up reporting on inputs while a model quietly gets worse: an input distribution to compare against a reference (**data drift**), and a real, timely ground-truth signal to tell whether the model's predictions are actually still correct (**concept drift** — the one that matters). This project used to run an Evidently service watching raw sensor telemetry over Kafka, but it had neither ingredient for real: its reference distribution was seeded random noise (there was never an actual telemetry producer feeding it), and it explicitly scoped concept drift out for a harder, structural reason — RUL's ground truth is an engine's actual failure or maintenance event, which arrives sparsely, on a timescale of weeks to months per engine, and is often censored (most engines under observation are never run to actual failure). A system that reports "drift detected" against a synthetic input stream with no real feedback loop behind it is worse than no monitoring at all, because it's believed.

**What's here instead.** A structured JSON log line per inference request (`structlog`, shipped to CloudWatch — directly queryable in CloudWatch Logs Insights, no text parsing required), and the same offline quality gate every training run already has to clear before it can reach `champion`. Neither one is drift monitoring, and neither claims to be.

**What would have to be true for this to come back.** A real telemetry source producing this project's actual input distribution — not a random-noise simulator — and a real, timely channel connecting a served prediction back to what actually happened to that engine. Data drift alone is buildable without the second piece; concept drift specifically is not, and building it without a real feedback loop is exactly the failure mode this section is refusing to ship.
...
**A Feature Store used for its offline store only, not for online serving.** Feast supports an offline (Parquet/S3) + online (Redis) split precisely so training and low-latency inference can each be optimized for what they actually need. This project used to run both, materializing the online store from a Kubernetes CronJob every 30 minutes — infrastructure that only earns its cost if something actually serves inference from streaming data. Nothing here does (see [Monitoring & Observability](#monitoring--observability) for the sibling decision that made that true), so the online store, the CronJob, and ElastiCache were removed rather than kept running unused. What remains — `get_historical_features` against the offline store — is still what eliminates training/serving skew for this project: training reads the exact same feature *definitions* (`feature_store/features.py`) that generated the data the model was evaluated on, point-in-time-correct, by construction.
...
**Testcontainers + LocalStack over mocking external systems in integration tests.** A test that mocks Postgres or S3 can pass while the real integration is broken — the mock and the real system silently drift apart. Testcontainers spins up real, ephemeral containers from pytest itself; LocalStack does the same for AWS (S3, Secrets Manager) at zero cost and zero credentials. These are slower than mocked tests, so they're excluded from the default `make test` / pre-commit run and executed explicitly via `make test-integration` — a deliberate trade of speed for confidence, made only where the integration actually matters.

**Two independent Poetry projects (`api/`, `core_ml/`) instead of one shared dependency tree.** The serving API and the training pipeline have very different dependency footprints — Feast, DVC, and PyTorch's training-time surface have no reason to ship inside the production API image. Splitting them keeps that image's dependency surface, build time, and attack surface minimal, at the cost of some duplicated boilerplate (`config_loader.py`, `logging_config.py`) between the two projects — a trade made deliberately in favor of the API's small footprint.
...
├── core_ml/                     # Data pipeline, feature store & training (own Poetry project)
│   ├── src/
...
│   ├── feature_store/           # Feast entity + feature view definitions (S3 registry, offline store only)
...
├── kubernetes/
│   ├── base/                    # Kustomize base: api / mlflow Deployments + ExternalSecret +
...
├── terraform/                   # AWS infra (VPC, EKS, RDS, S3, ECR, KMS, IAM/OIDC)
...
```bash
make compose-up     # Postgres, MLflow, the API, and LocalStack
...
```bash
make test-integration   # requires Docker; Testcontainers spins up real Postgres/LocalStack
```

These are excluded from `make test` and from the pre-commit hooks (`-m "not integration"`), so the fast loop stays fast. They cover the API serving from a real MLflow registry on Postgres, and S3/Secrets Manager against LocalStack.

### 7. Release a new version (GitOps) and validate infrastructure locally

```bash
make docker-build              # builds the images: api + mlflow
...
`make docker-build` is the local path (Docker Desktop already caches layers between builds). CI uses `make docker-buildx-push` instead: the same images, built through a `docker-container` buildx driver with a BuildKit registry cache in ECR, and skipped entirely if the tag already exists — which is what makes retrying a partially-failed `build_image` safe against immutable ECR repos.
...
- **Drift monitoring, once there's something real to monitor.** See [Monitoring & Observability](#monitoring--observability) for why this was removed rather than kept running against synthetic input and an absent ground-truth signal — the precondition for bringing it back is a real telemetry source and a real, timely feedback channel from a served prediction back to what actually happened to that engine, not a smaller engineering task than it sounds.
...
- **Load testing the serving path.** There is no formal latency/throughput benchmark for `/predict` under sustained load; this would inform right-sizing the EKS node group.
- **Multi-region / disaster recovery.** The current Terraform footprint targets a single AWS region; RDS cross-region replication and a documented failover runbook are not yet in place.
